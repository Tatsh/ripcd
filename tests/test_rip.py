...
def _musicbrainz_disc_response(
        *,
        ...
        album: str = 'TestAlbum',
        year_date: str = '2023',
        track_titles: tuple[str, ...] = ('Track1', 'Track2'),
) -> dict[str, object]:
...
def _patch_musicbrainz_success(
    ...
    mocker.patch(
        'ripcd.rip.musicbrainzngs.get_releases_by_discid',
...
async def test_rip_cdda_to_flac_falls_back_to_cddb(mocker: MockerFixture, tmp_path: Path) -> None:
    ...
    await rip_cdda_to_flac(drive='/dev/cdrom', output_dir=tmp_path, username='user')
    cddb_mock.assert_called_once_with(
        'fake_cddb_query',
...
@pytest.mark.asyncio
async def test_rip_cdda_to_flac_cddb_fallback_honours_accept_first(
    mocker: MockerFixture,
    tmp_path: Path,
    *,
    accept_first: bool,
