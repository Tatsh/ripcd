...
If any changed files are under `ripcd/` or `tests/`, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `ripcd/`, `tests/`,
...
For example, if a commit contains `ripcd/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`ripcd/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `ripcd/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `ripcd/` prefix and replace `/` with `.` (like module imports).

- Python file `ripcd/media.py` → `media:`.
- Multiple files under `ripcd/commands/` → `commands:`.
- Single command file `ripcd/commands/admin.py` → `commands.admin:`.
