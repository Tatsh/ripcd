---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the ripcd project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the ripcd project.
...
- `` :py:mod:`ripcd.string` `` for modules
- `` :py:func:`ripcd.string.slugify` `` for functions
- `` :py:class:`ripcd.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~ripcd.string.slugify` `` renders as `slugify`
